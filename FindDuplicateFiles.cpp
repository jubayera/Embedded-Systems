/*
 * We walk through our whole file system iteratively. As we go, we take a "fingerprint" of each file in constant time by hashing
 * the first few, middle few, and last few bytes. We store each file's fingerprint in an unordered map as we go.
 * 	If a given file's fingerprint is already in our unordered map, we assume we have a duplicate. In that case, we assume the file
 * 	edited most recently is the one created by our friend.
 */
#include <cstring>
#include <algorithm>
#include <iomanip>
...
class FilePaths
{
	public:
		...
		{
			ostringstream str;
			str << "(original: " << originalPath_	<< ", duplicate: " << duplicatePath_ << ")";
			return str.str();
		}
};
...
//We wrap our directory handle in a class to make it exception safe and ensure the directory always gets closed.
...
vector<unsigned char> sampleFileData(const string& path)
{
	...
	//reverse storage of the appropriate size for file data
	vector<unsigned char> fileData(dataSize);

	//Read the file using low-level system call based file I/O. C++ STL may not properly handle files larger than 2 GB.
	...
	if(fd != -1) {
		...
		/* If size is too small to take 3 samples, read the entire file into the provided buffer, updating readLength. */
		if(dataSize <= MAX_DATA_SIZE) {
			readLength = read(fd, static_cast<void*>(&fileData[0]), dataSize);
		}

		//Otherwise, read the samples into the buffer at the right offsets, and update the readLength.
		else {
			readLength += read(fd, static_cast<void*>(&fileData[0]), BLOCK_SIZE);
			
			if(lseek(fd, (st.st_size / 2) - (BLOCK_SIZE / 2), SEEK_SET) == (off_t) - 1) {
				throw runtime_error("Can't lseek file.");
			}

			readLength += read(fd, static_cast<void*>(&fileData[BLOCK_SIZE]), BLOCK_SIZE);

			if(lseek(fd, -BLOCK_SIZE, SEEK_END) == (off_t) - 1) {
				throw runtime_error("Can't lseek file.");
			}

			readLength += read(fd, static_cast<void*>(&fileData[BLOCK_SIZE * 2]), BLOCK_SIZE);
		}
